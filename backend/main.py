...
from backend.database import get_db, KnowledgeBase, ChatHistory, init_db
...
def startup_event():
    ...
    logger.info("✅ Chroma storage directory ready")
...
async def upload_document(
    ...
    logger.info(f"📂 Upload → project: {project_name}")
    try:
...
async def recent_history(db: Session = Depends(get_db)):
    ...
    return [
        ResearchResponse(query=e.query, result=e.response, file_path=None, suggestions=[])
        for e in logs
    ]
